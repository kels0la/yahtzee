...
export const TableRow = props => {
  ...
  let { rowType, radioBtnValue, labelText, bottomFormatting, disabledScores, scores, disabledTakeScoreBtn, selectedOption, theme } = props;
  let rowOneClasses, rowTwoClasses, rowThreeClasses, rowThreeDisplay, scoreDisplay, rowThree;
  let bottomBorder = `border-b-2 ${theme.specialBorder} pb-1`;
  ...
  function firstRowFormatting() {
    ...
    rowThreeDisplay =
      <Button2
        classprops={`outline-button-two p-0 px-1 ${theme.darkestBorder} border rounded text-sm ${theme.specialBackground} h-6`}
        type="submit"
  ...
  return (
    <React.Fragment>
      <div className={`${theme.standardText} table-row w-full flex text-xs`}>
        <div className={`w-1/3 ${theme.standardText} table-cell pt-1 ${rowOneClasses}`}>{labelText}</div>
        <div className={`w-1/3 ${theme.standardText} table-cell pt-1 text-center ${rowTwoClasses}`}>{scoreDisplay}</div>
        {rowThree}
      </div>
    </React.Fragment>
  );
