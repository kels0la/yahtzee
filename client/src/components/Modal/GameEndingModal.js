...
export const GameEndingModal = props => {
  ...
  let modalDisplay;
  let { totalScore, theme } = props

  if (props.showEndGameModal) { modalDisplay =
      <div className='overlay'>
        <div className='w-1/3'></div>
        <div className={`w-1/3 modalTop ${theme.darkestBackground} ml-auto mr-auto p-3 border ${theme.standardBorder} ${theme.shadowEffect} pin-x absolute rounded-lg mt-32 z-10 mb-3`}>
          <h2 className={`mb-3 text-center ${theme.standardText}`}>Your final score was: <span className={`${theme.specialText}`}>{totalScore}</span></h2>
          ...
          <p className={`mb-1 text-center ${theme.standardText}`}>Click <span className={`${theme.specialText}`}>"Start New Game"</span> to start a new game. Or...</p>
          <p className={`mb-3 text-center ${theme.standardText}`}>Click <span className={`${theme.specialText}`}>"Bask in Glory"</span> to stare at your score a little while longer.</p>
          <hr className={`${theme.specialBorder} border outsetBorder mb-3`}></hr>
