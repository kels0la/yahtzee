  ...
  return (
    <React.Fragment>
      <div onClick={(event) => handleBackgroundClick(event)} className='overlay'>
        ...
        <div className={`w-1/3 modalTop ${theme.background} ml-auto mr-auto p-3 border ${theme.standardBorder}
         ${theme.shadowEffect} pin-x absolute rounded-lg mt-32 z-10 mb-3`}>
          ...
          <div className='justify-between flex'>
            <ResetButton
              classprops={`main-button outline-button p-0 inline-block px-1 ${theme.darkestBorder}
              border rounded text-sm ${theme.specialBackground}`} text='New Game' styles={`${theme.darkText} font-bold`}
              onClick={(event) => props.resetGame(event)} />
            <Button2
              classprops={`outline-button-two p-0 px-1 ${theme.darkestBorder} border rounded text-sm
              ${theme.lightBackground} h-6`}
